...
class DownloaderHelper(_IPluginModule):
    ...
    def init_config(self, config=None):
        ...
        if (self.__config_obj.get('run_once')):
            if (self.__check_has_enabled_task()):
                self.__init_scheduler(timezone)
                self.__scheduler.add_job(self.__do_task, 'date',
    ...
    def get_state(self):
        ...
        state = True if self.__config_obj \
                        ...
                        and (self.__config_obj.get('cron') or self.__config_obj.get(
            "listen_download_event") or self.__config_obj.get("listen_source_file_event")) \
    ...
    def __check_need_delete(cls, torrent, downloader_type, deleted_source_file=None):
        ...
        is_match, torrent_data_path = cls.__check_file_match_torrent(source_path, source_filename, torrent,
                                                                     downloader_type)
        if (not is_match):
            return False
    ...
    def __delete_one_for_qb(self, torrent, downloader, deleted_source_file=None):
        ...
        if (self.__exists_exclude_tag(torrent_tags)):
            return False
        downloader_type = DownloaderType.QB
        if (not self.__check_need_delete(torrent=torrent, downloader_type=downloader_type,
    ...
    def __delete_batch_for_qb(self, downloader, torrents, deleted_source_file=None):
        ...
        for torrent in torrents:
            if (self.__exit_event.is_set()):
                return count
            if (
            self.__delete_one_for_qb(torrent=torrent, downloader=downloader, deleted_source_file=deleted_source_file)):
                count += 1
        self.info('[QB]批量删种结束')
        return count
    ...
    def __delete_batch_for_tr(self, downloader, torrents, deleted_source_file=None):
        ...
        for torrent in torrents:
            if (self.__exit_event.is_set()):
                return count
            if (
            self.__delete_one_for_tr(torrent=torrent, downloader=downloader, deleted_source_file=deleted_source_file)):
                count += 1
        self.info('[TR]批量删种结束')
        return count
    ...
    def __delete_batch(self, downloader, torrents=None, deleted_source_file=None):
        ...
        if (not torrents):
            torrents = self.__get_torrents(downloader)
        if (downloader_type == DownloaderType.QB):
            return self.__delete_batch_for_qb(downloader=downloader, torrents=torrents,
                                              deleted_source_file=deleted_source_file)
        elif (downloader_type == DownloaderType.TR):
            return self.__delete_batch_for_tr(downloader=downloader, torrents=torrents,
                                              deleted_source_file=deleted_source_file)
        return 0
    ...
    def __do_task_for_all_downloader(self, sub_task_control=None, deleted_source_file=None):
        ...
        for downloader_id, downloader_config in downloader_configs.items():
            result = self.__do_task_for_single_downloader(downloader_id=downloader_id,
                                                          downloader_config=downloader_config, site_dict=site_dict,
                                                          sub_task_control=sub_task_control,
                                                          deleted_source_file=deleted_source_file)
            if (result):
    ...
    def __do_task_for_single_downloader(self, downloader_id=None, downloader_config=None, site_dict=None,
                                        sub_task_control=None, deleted_source_file=None):
        """
        针对单个下载器执行任务
        """
        if (self.__exit_event.is_set()):
        ...
        if (not downloader_config.get('enable')):
            return None
        # 子任务开关处理
        enable_seeding = downloader_config.get('enable_seeding') == True and (
                    sub_task_control == None or sub_task_control.get('enable_seeding') != False)
        enable_tagging = downloader_config.get('enable_tagging') == True and (
                    sub_task_control == None or sub_task_control.get('enable_tagging') != False)
        enable_delete = downloader_config.get('enable_delete') == True and (
                    sub_task_control == None or sub_task_control.get('enable_delete') != False)
    ...
    def __get_site_dict(self):
        ...
        if (site_infos):
            return {self.__parse_keyword_from_url(site_info.get("signurl")): site_info.get("name") for site_info in
                    site_infos}
        return {}
    ...
    def listen_download_add_event(self, event):
        ...
        timezone = Config().get_timezone()
        self.__scheduler.add_job(__do_task, 'date',
                                 run_date=datetime.now(tz=pytz.timezone(timezone)) + timedelta(seconds=3))
